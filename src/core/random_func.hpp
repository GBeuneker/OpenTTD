 ...
 */
static inline bool Chance16I(const uint16_t a, const uint16_t b, const uint32_t r)
{
	assert(b != 0);
...
#else
static inline bool Chance16(const uint16_t a, const uint16_t b)
{
...
#else
static inline bool Chance16R(const uint16_t a, const uint16_t b, uint32_t &r)
{
	r = Random();
